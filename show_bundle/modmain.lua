...
ENV.modimport("main/util.lua")
ENV.modimport("main/showbundle_widgetcreation.lua")
...
local function send_showbundle_request(target)
    ENV.SendModRPCToServer(MOD_RPC[ENV.modname]["ShowBundle"], target)
...
-- RPC data handler
ENV.AddClientModRPCHandler(ENV.modname, "ShowBundleCallback", function(target, data)
    target.showbundle_itemdata = SB.unserialize(data) or {}
    draw_tipbox(target.showbundle_itemdata, target)
end)

ENV.AddModRPCHandler(ENV.modname, "ShowBundle", function (player, target)
    ...
    ENV.SendModRPCToClient(ENV.CLIENT_MOD_RPC[ENV.modname]["ShowBundleCallback"], player.userid, target, SB.serialize(itemdata))
end)
...
if ShowMe_Hint then
    ...
    function SendModRPCToServer(code, GUID, ...)
        ...
        if code == ShowMe_Hint
        and ent
        and (ent.prefab == "bundle" or ent.prefab == "gift")
        then
            return
        end
